    ...
    function update_user() {
        submit_button.addEventListener('click', function () {
           ...
           /* if (user_image == "none") {
            ...
            if (image_updated === 0){
                user_image = uploaded_image.data("value");
            }
            $.ajax({
                headers: {
    ...
    function success_submit() {
        ...
        (submit_button.setAttribute("data-kt-indicator", "on"), submit_button.disabled = !0, setTimeout((function () {
            submit_button.removeAttribute("data-kt-indicator"), Swal.fire({
                text: language === "en" ? "The Edit saved successfully!" : "تم حفظ التعديل بنجاح!",
                icon: "success",
                buttonsStyling: !1,
                confirmButtonText: language === "en" ? "Ok, got it!" : "حسنًا ، فهمت!",
                customClass: {confirmButton: "btn btn-primary"}
            }).then((function (e) {
                e.isConfirmed && (window.location = add_user_form.attr("data-kt-redirect"));
            }))
