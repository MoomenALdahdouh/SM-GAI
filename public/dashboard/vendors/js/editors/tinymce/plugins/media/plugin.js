...
tinymce.PluginManager.add('media', function(editor, url) {
	...
	function showDialog() {
		...
		win = editor.windowManager.open({
			...
			onSubmit: function() {
				...
				beforeObjects = editor.dom.select('web_page_img[data-mce-object]');
				editor.insertContent(dataToHtml(this.toJSON()));
				afterObjects = editor.dom.select('web_page_img[data-mce-object]');
	...
	function dataToHtml(data) {
		...
		if (data.embed) {
			...
			if (data.type == "iframe") {
				...
				if (data.poster) {
					html += '<web_page_img src="' + data.poster + '" width="' + data.width + '" height="' + data.height + '" />';
				}
	...
	editor.addButton('media', {
		...
		onclick: showDialog,
		stateSelector: ['web_page_img[data-mce-object]', 'span[data-mce-object]']
	});
